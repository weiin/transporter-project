...
void ImuNode::publish_imu_data()
{
	...
	// so need to convert to East-North-Up (ENU) coordinates according to ROS REP103
  ...
  trans.setRotation(tf::Quaternion(imu_data.orientation.x,imu_data.orientation.y,imu_data.orientation.z,imu_data.orientation.w));
  br.sendTransform( tf::StampedTransform (trans.inverse(), imu_data.header.stamp, "base_link", "compass") );
